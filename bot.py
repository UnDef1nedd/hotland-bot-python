...
async def on_ready(): # Объявление асинхронной функции
    print (f"Залогинен как {settings['NAME BOT']}") # Сообщение в консоль при готовности бота (https://prnt.sc/uog9bz)

    await client.change_presence(status=discord.Status.online, activity=discord.Streaming(name=f'Приветик!', url='https://www.twitch.tv/unknowpage')) # Статус бота (Для примера выбрал стриминг)
